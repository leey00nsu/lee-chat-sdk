...
The default UI includes `sending`, `failed`, retry, assistant loading, presence, typing, and read receipt states. Customize copy through `config.texts`, class hooks through `config.className`, and replace message rendering in React with `LeeChatWidget` `renderMessage` and `renderAssistantLoading`.
